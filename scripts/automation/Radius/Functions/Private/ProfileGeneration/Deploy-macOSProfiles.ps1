function Deploy-macOSProfiles {
    ...
    begin {
        ...
        # Get the API key and org ID from environment variables
        $apiKey = $env:JCApiKey
        $orgID = $env:JCOrgId

        # Verify API credentials are available
        if (-not $apiKey -or -not $orgID) {
            Write-Error "JumpCloud API key or Organization ID is missing. Please ensure you've connected to JumpCloud using Connect-JCOnline."
            return $false
        }
        ...
        foreach ($user in $userObject) {
            ...
            if (Test-Path -Path "$userProfilePath") {
                ...
                if ($user.macOSProfile -eq $null) {
                    ...
                    if ($user.macOSProfile.profilePayloadDisplayName -eq $null) {
